  ...
  <div class="right">
    ...
    <div class="search">
      <c:if test="${role_select}">
        <form  method="post" action="roleSelect">
          <span>查询条件：</span>
          <input type="text" placeholder="请输入待查询的名字"/>
    ...
    <table class="providerTable" cellpadding="0" cellspacing="0">
      <tr class="firstTr">
        ...
        <c:if test="${role.rid!=0}" >
          <tr>
            <td>${role.ucode}</td>
            <td>${role.uname}</td>
            <td>
              <c:if test="${role.rid==1}">超级管理员</c:if>
              <c:if test="${role.rid==2}">普通管理员</c:if>
            </td>
            <td>
