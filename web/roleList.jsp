...
<%
    ...
    List<T_priviage> priList = userInfo.getT_priviageList();
        ...
        <div class="right">
            ...
            <div class="search">
                ...
                <c:if test="${role_select}">
                    <form style="float:left" method="post" action="roleSelect">
                    <span>查询条件：</span>
                    <input  name="uname" type="text" placeholder="请输入待查询的名字"/>
            ...
            <table class="providerTable" cellpadding="0" cellspacing="0">
                ...
                <tr>
                    ...
                    <td>${role.uname}</td>
                    <td>
                        <c:if test="${role.id==1}">超级管理员</c:if>
                        <c:if test="${role.id==2}">普通管理员</c:if>
                        <c:if test="${role.id==3}">普通用户</c:if>
                    </td>
                    <td>
                        <c:if test="${role_details}">
                        <a href="userView?ucode=${role.ucode}"><img src="img/read.png" alt="查看" title="查看"/></a>
                        </c:if>
                        <c:if test="${role_edit}">
                        <a href="roleUpdate?ucode=${role.ucode}"><img src="img/xiugai.png" alt="修改" title="修改"/></a>
                        </c:if>
                        <c:if test="${role_delete}">
                        <a href="roleDelete?ucode=${role.ucode}" class="removeUser"><img src="img/schu.png" alt="删除" title="删除"/></a>
                        </c:if>
                    </td>
                </tr>
                </c:if>
